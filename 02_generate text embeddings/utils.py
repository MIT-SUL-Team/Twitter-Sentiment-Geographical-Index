...
def encode_text(p_df, p_model="stsb-xlm-r-multilingual", p_max_len=128, p_batch_size=100, drop_duplicate=False):
    mdf = p_df.copy()

    print("Cleaning training data")
    mdf['label'] = [0 if x == 0 else 1 for x in mdf['label']]
    mdf['lang'] = 'en'
    mdf['text'] = [clean_for_content(text, lang) for text, lang in
                   tqdm(zip(mdf['text'], mdf['lang']), total=mdf.shape[0])]

    mdf = mdf[mdf['text'] != ''].reset_index(drop=True)

    if drop_duplicate:
        mdf = mdf.drop_duplicates(subset=['text'])

    print("Creating Embeddings")
    encoder = SentenceTransformer(p_model)
    encoder.max_seq_length = p_max_len

    embeddings = encoder.encode(mdf['text'].values, show_progress_bar=True, batch_size=p_batch_size)

    return embeddings, encoder, mdf
